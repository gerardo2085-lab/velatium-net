const c=window.API,p=sessionStorage.getItem("literacy_token");function y(t){["access","catalog","themes"].forEach(n=>{document.getElementById(`panel-${n}`).style.display=t===n?"block":"none",document.getElementById(`tab-${n}`).classList.toggle("active",t===n)})}window.showTab=y;async function o(t,n="GET",e=null){const s={method:n,headers:{"X-Session-Token":p}};return e&&(s.headers["Content-Type"]="application/json",s.body=JSON.stringify(e)),fetch(`${c}${t}`,s)}async function d(){const e=(await(await o("/admin/users")).json()).filter(s=>s.status==="pending");if(!e.length){document.getElementById("users-list").innerHTML='<p style="color:var(--text-muted);font-size:13px;">No pending requests.</p>';return}document.getElementById("users-list").innerHTML=e.map(s=>`
        ...
        <div class="book-row">
          <img src="/images/covers/${e.slug}.jpeg" onerror="this.style.display='none'"
        ...
        </div>`).join("")}window.setTheme=async t=>{await o("/config/theme","POST",{theme:t}),document.getElementById("theme-css").href=`/themes/${t}.css`,l()};d();r();l();
