const c=window.API,p=sessionStorage.getItem("literacy_token");function h(n){["access","catalog","themes"].forEach(t=>{document.getElementById(`panel-${t}`).style.display=n===t?"block":"none",document.getElementById(`tab-${t}`).classList.toggle("active",n===t)})}window.showTab=h;async function a(n,t="GET",s=null){const e={method:t,headers:{"X-Session-Token":p}};return s&&(e.headers["Content-Type"]="application/json",e.body=JSON.stringify(s)),fetch(`${c}${n}`,e)}const y={pending:"Pending",active:"Active",rejected:"Rejected"},v={pending:"var(--text-secondary)",active:"var(--text-primary)",rejected:"var(--text-muted)"};async function d(){const t=await(await a("/admin/users")).json();if(!t.length){document.getElementById("users-list").innerHTML='<p style="color:var(--text-muted);font-size:13px;">No users yet.</p>';return}const s={pending:0,active:1,rejected:2};t.sort((e,o)=>(s[e.status]??9)-(s[o.status]??9)),document.getElementById("users-list").innerHTML=t.map(e=>`
        <div class="user-row">
          ...
          <div class="row-actions" style="flex-shrink:0;">
            <span class="badge" style="color:${v[e.status]||"inherit"}">${y[e.status]||e.status}</span>
            ${e.status==="pending"?`
        ...
        </div>`).join("")}window.setTheme=async n=>{await a("/config/theme","POST",{theme:n}),window.__applyTheme&&window.__applyTheme(n),u()};function f(){document.getElementById("users-list")&&(d(),r(),u())}document.addEventListener("astro:page-load",f);
